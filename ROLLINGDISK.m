...
d = 0.3;              % Distance from the center of mass to the weight (m)

% Initial conditions
phi0 = pi/4;             % Initial angle (rad)
phi_dot0 = -1;        % Initial angular velocity (rad/s)

% Time span
tspan = [0 7];        % Time from 0 to 5 seconds

% Define the system of ODEs
odefun = @(t, y) [y(2); 
                  -((m + m_w) * (g * c * cos(y(1)) + 2 * R * c * cos(y(1)) * y(2)^2)) ...
                   / ((m + m_w) * (R^2 + 2 * R * c * sin(y(1)) + c^2) + (1/2) * m * R^2 + m * c^2 + (1/2) * m_w * R_w^2 + m_w * d^2)];
